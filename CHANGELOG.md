...
- **Live preview no longer fails silently after an in-place upgrade** ([VIB-1937](/VIB/issues/VIB-1937)) — when `/api/preview-origin` can't be resolved (a stale cached `ui/preview.js` hitting a moved route after an upgrade, the preview-origin process not running, or its port blocked), `refreshPreview` used to only `console.error` and leave a blank frame. It now renders a visible in-frame fallback (trusted `srcdoc`) explaining the preview is unavailable, with two concrete fixes: hard-refresh the page (to pick up the new client after an upgrade) and ensure the preview port — app port + 2, e.g. 4202 — is free and reachable. The stale-cache root cause itself is already defended by the `no-store`/`no-cache` headers on `index.html` and the `ui/*.js` bundle (VIB-108), now locked by `test/preview-fallback-cache.test.ts` so an upgraded browser always re-fetches the current client.
- **Editor URL reload no longer discards inline field edits** ([VIB-1935](/VIB/issues/VIB-1935)) — reloading the tab (or navigating to `#/app/<theme>`) hits `POST /api/setup/open`, which always rebuilt a fresh session from the theme folder on disk. Field edits live only in the session store (`updateFieldValue` never writes back to `fields.json`), so they were silently reverted to the on-disk defaults. The open route now resumes instead of rescanning: it keeps the live in-memory session when it's already for that theme, otherwise loads the most recently updated saved session for the theme path — the same semantics as the setup screen's "continue where you left off" card. A fresh disk scan still happens when no session exists. Covered by `test/setup-open-resume.test.ts`.
- **Docker/HTTPS deployments: the live preview works again** ([VIB-1933](/VIB/issues/VIB-1933)) — since VIB-1892 the preview loads from a separate origin at app port + 2, but the compose bundle neither exposed port 4202 nor proxied it, so `/api/preview-origin` announced an unreachable (and, on https pages, mixed-content-blocked) `http://<host>:4202` and the preview pane was dead in every documented Docker topology. The public URL of the preview origin is now configurable via `VIBESPOT_PREVIEW_PUBLIC_ORIGIN` (scheme-aware; `/api/preview-origin` returns it verbatim), and the new `VIBESPOT_PUBLIC_ORIGIN` pins the preview's `frame-ancestors` CSP to the app origin instead of `*` on proxied binds. The compose bundle routes a second Caddy site (`preview.<VIBESPOT_DOMAIN>`, override with `VIBESPOT_PREVIEW_DOMAIN`) to `vibespot:4202` — deliberately not through the oauth2-proxy auth overlay (an iframe can't complete an OIDC redirect; the origin stays token-gated) — and all three Docker docs cover the new hostname, the LAN `4202:4202` mapping, and nginx/k8s equivalents.
...
- **UI editing data-loss fixes; the three field-save paths are unified** ([VIB-1898](/VIB/issues/VIB-1898)):
  ...
  - The checkpoint gate actually locks the composer: pressing Enter while a gate card was parked started a second pipeline run behind the gate (only the send buttons were disabled). A gate flag now blocks `sendMessage`, and a queued message stays queued (chip intact) until the gate resolves.
  - A generation error no longer strands the "Queued — runs next" chip forever — the queued message is dispatched when the failed run ends.
  - `refreshPreview`'s async origin lookup can no longer clobber a newer navigation or the generating screen (stale resolves are dropped).
- **Agent orchestration: cancellation, run-overlap, module reuse, blog wiring, rate-limit retry** ([VIB-1895](/VIB/issues/VIB-1895)):
  - Barge-in now actually stops CLI-engine work: the cancellation signal reaches the spawned `claude`/`gemini`/`codex` subprocesses and kills them immediately (previously they had no external kill hook and kept burning tokens until a 10-minute timeout). The signal also reaches Stage 1 (intent analyzer) and Stage 2 (design system / module planner / site planner) API calls, which previously billed to completion after a cancel.
  - Generations are serialized by a per-session lock instead of a global flag plus a 10-second "wait, then start anyway" poll — under a slow unwind, two (or three) pipelines could previously run against the same session at once, and the older run's cleanup cleared the newer run's generating flag mid-build. A barge-in now cancels every in-flight *and* queued run, and the replacement waits for the lock instead of overlapping.
  - Module reuse works: "Reuse: hero from Other page" previously announced the reuse and then silently dropped the module (the handler stripped the module payload the assembler needed, so the final order filter removed it). The payload now rides through the pipeline; a reuse whose payload is genuinely missing is logged instead of vanishing.
  - Blog templates are reachable: the intent analyzer offered `contentType: "blog"` in its schema but then clobbered it to `"page"`, leaving the whole blog path (blog prompts, schema, validator rules, blog_post template + listing writer) as dead code. "blog" now flows through the pipeline, the template is marked `blog_post` on apply (so disk writes emit the blog template + listing), and follow-up requests stay in blog mode — mirroring the email pattern.
  - OpenAI and Gemini rate-limit (429) responses now get the same backoff-retry (10s→120s) the Anthropic path already had; previously the adapters threw a status-429 error nothing consumed, producing one blind retry and a failed module. An abort during the backoff stops the retry loop.
  - A chat that supersedes a parked checkpoint gate now discards the gate's in-memory resume state (previously it leaked for the life of the process), and a module whose generation task rejects is reported under its real name instead of "unknown".
...
- **The live preview no longer runs AI-generated code on the app origin** ([VIB-1892](/VIB/issues/VIB-1892)) — the preview iframe used `srcdoc` with `sandbox="allow-scripts allow-same-origin"`, which on `srcdoc` inherits the app origin and neuters the sandbox: a `<script>` in generated (or prompt-injected / imported-theme) page code could reach the app's DOM and every `/api/*` route — rewrite the UI, flip settings, read the masked API-key state, drive uploads. Now:
  ...
  - The composed doc ships under a strict CSP (`connect-src 'none'` — generated code cannot fetch/XHR/WebSocket anywhere; `frame-ancestors` pinned to the app; `base-uri`/`form-action 'none'`).
  - All in-preview interaction (inline text/image/link editing, section controls, change highlights, working overlays) moved into a trusted agent (`ui/preview-agent.js`) injected ahead of any module script, talking to the app over a narrow, origin- and token-checked `postMessage` protocol (`src/server/preview-protocol.ts`). The only write verbs that cross the boundary are module **field edits** — a fully compromised preview can at most change a field value on the open theme, never touch settings, keys, or uploads.
  - The dashboard module preview and email preview iframes drop `allow-same-origin`, so their `srcdoc` content now runs in a genuinely opaque origin.
  - **Esc now exits interact mode from inside the preview** — a first Esc closes an open inline editor, a second one leaves interact mode. The frame only *asks* (read-only `vs:request-mode`, honoured solely as an exit-to-view request); the parent flips its own toolbar state and answers with `vs:set-mode`, so the write surface is unchanged.

- **AI output and HubSpot metadata can no longer write outside the theme** ([VIB-1891](/VIB/issues/VIB-1891)) — two arbitrary-file-write primitives closed:
  - Module names from the planner's structured output (and page ids/slugs and shared-module names from the intent analyzer / site planner) are kebab-sanitized the moment they leave the model, on every path including approve/one-shot — a hostile name like `../../../home/user/.config/autostart/x` can no longer become a filesystem path. Every filesystem sink that builds `modules/<name>.module` (write, reload, delete — including the `rm -rf` behind module/template deletion, reachable with a client-supplied name) additionally verifies the resolved path stays a direct child of the theme's `modules/` directory (`src/utils/path-safety.ts`).
  - `fetchTheme` (HubSpot theme import) now refuses server-supplied file paths that resolve outside the download target directory (zip-slip via `../../..` in the CMS Source Code API listing).
  - Template-file sinks got the same sink-level containment ([VIB-1912](/VIB/issues/VIB-1912)) — writing templates to disk skips-and-warns on any computed filename (`tpl.id`, email `templateFile`) that isn't a plain path segment, and reloading a template from disk refuses a `templateFile` that resolves outside the theme directory, so a poisoned persisted session can neither write nor read outside the theme via template entries.
...
- **The local server is no longer an open door** ([VIB-1889](/VIB/issues/VIB-1889)) — previously the HTTP API and chat WebSocket bound to `0.0.0.0` with no authentication, so anyone who could reach the port could spend your AI credits, push to your connected HubSpot portal, and browse local directories. Now:
  ...
  - Error responses no longer echo raw filesystem paths/usernames from underlying OS errors.
...
- **Brand-intake checkpoint — match your brand, or be surprised** ([VIB-1878](/VIB/issues/VIB-1878)) — a front-of-flow ask-back that fires at the very start of a build, *before* the design system, but **only when there's no style system to work from** — a brand-new page with no extracted styleguide and no `:root` in the shared CSS. An imported HubSpot theme or a resumed session already carries a style system, so vibeSpot uses it and never asks. When it does ask, you pick **Surprise me** (the AI invents the design system — exactly today's behaviour) or **Bring your brand**: paste brand colors, paste CSS/HTML, describe your tone of voice, point at a public site URL, or point at a local HubSpot theme. Each channel is routed to its existing extractor (colors/code → token parse, theme → inverse-analyzer, site → fetch + parse, voice → brandvoice) and folded into a `:root` plus a brand brief that seeds the design system — so the very next **design checkpoint** ([VIB-1877](/VIB/issues/VIB-1877)) renders *your* palette and type, and the brand is honoured through the build and inherited by future edits. All channels are deterministic (no extra model call), so the gate stays cheap. Reuses the same checkpoint-gate primitive as the design checkpoint, with a richer card. This is the second slice of the conversational pipeline ([VIB-1876](/VIB/issues/VIB-1876)).
...
- **Structure checkpoint — shape the module skeleton before building** ([VIB-1879](/VIB/issues/VIB-1879)) — a second seam on the same checkpoint primitive, immediately after the module planner (Stage 2b, the `blueprint_ready` point) and before the parallel build. Once you approve the design, the pipeline parks again and shows a **structure checkpoint card**: the planned modules as an **editable outline** you can **reorder** (▲/▼), **cut** (✕), **rename** (inline), and **add to** (+ Add section) — all client-side, no model call. **approve/skip** builds exactly the outline you kept (Stage 3 honors your edits — a reorder/rename preserves each module's planned brief; an added row gets a sensible default brief), **steer** re-runs *only* the module planner with your note and re-previews, **cancel** writes nothing. The flow is now design gate → structure gate → build; **skip** at the design gate suppresses the structure gate too, and the one-shot send button still bypasses both. Third slice of the conversational pipeline ([VIB-1876](/VIB/issues/VIB-1876)).
