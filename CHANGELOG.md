...
- **Settings panel timing out ("Settings unavailable")** ([VIB-1834](/VIB/issues/VIB-1834)) — the AI Settings panel could fail to load with *"Settings took too long to load. The server may be busy or unreachable."* The `/api/settings/status` endpoint was blocking on two unbounded slow paths while the client aborted after only 3s: (1) the live model-catalog lookups (`api.anthropic.com`, `api.openai.com`, `generativelanguage.googleapis.com`) used bare `fetch()` with **no timeout**, so a slow/blocked/unreachable provider API (firewall, proxy, provider outage) stalled the whole response — this bites whenever a provider API key is configured; (2) the network-validating credential probes `gh auth status` and `hs accounts list` ran under the shared 120s subprocess ceiling. Each provider fetch now has a 2.5s `AbortController` timeout (with an aggregate race as a backstop), partial results degrade gracefully to the static model list, and only a fully-resolved catalog is cached. The credential probes are capped at 3s (a timeout reads as "not authenticated", same as any other failure). The endpoint's bounded worst case (~6s API mode / ~9s CLI mode) now sits comfortably under the client load budget, which was raised from 3s to 12s so a slow-but-recovering load shows the panel instead of a false error.
