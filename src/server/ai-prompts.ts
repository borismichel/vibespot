...
/**
 * Build the system prompt for vibe coding mode as a single string
 * (OpenAI/Gemini/CLI engines). Derived from the block builder so every
 * provider gets the identically-assembled prompt — previously this function
 * re-inlined the core instructions and had drifted in section order and in
 * always including the edit-mode section (VIB-1902).
 */
export function buildVibeSystemPrompt(
  ...
  return buildVibeSystemPromptBlocks(conversionGuide, themeName, editMode, pageType, brandAssets)
