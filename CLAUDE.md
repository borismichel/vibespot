...
A reusable **park-and-re-enter** primitive that pauses the pipeline at a cheap seam so the user can intervene before the expensive build — generalizing the older `plan_approve` pause. Shipped seams (in order): the **brand-intake checkpoint** (VIB-1878, C2, raised first when no style system exists), the **design checkpoint** (VIB-1877, C1), and **plan mode** as the heaviest variant (VIB-1880, C4, see below).
...
- **Brand-intake checkpoint (VIB-1878, C2):** a `brand_intake` gate raised *before* the design system, **only** on `intent === "create"` when `hasStyleSystem(snapshot)` is false (no `brandAssets.styleguide` and no `:root` in `sharedCss`) — an imported theme / resumed session has a style system, so the ask is skipped (Boris-locked "use it if present"). The card (`buildBrandIntakePreview()`, deterministic) offers **Surprise me** (resolved as `skip` → AI invents the design system, today's behavior) or **Bring your brand** (`approve` + `CheckpointResolution.brandIntake` channels). `resumeBrandIntake` (in `pipeline.ts`) routes the channels through `agent/brand-intake.ts` `routeBrandIntake()` — colors/code → in-memory token parse, `themePath` → `inverse-analyzer.extractDesignTokens`, `siteUrl` → fetch + parse, `voice` → brandvoice — folds them into one `:root` + brand brief, seeds a snapshot copy's `brandAssets.styleguide`/`brandvoice` + `sharedCss`, runs `runDesignSystem` (brand-aware), **merges the brand `cssVariables` (brand wins) + appends a brand override `:root`** so the design checkpoint renders the brand, then re-parks at the design gate. Derived brand assets ride back on `PipelineResult.brandAssetsUpdate` and are persisted by `handleAgenticResume` via `saveBrandAssetToTheme` (so future edits inherit them). All channels are deterministic — no model call at the gate. *(Deferred: file/screenshot upload → AI-vision styleguide, needs upload plumbing.)*

### Barge-in + plan fold (VIB-1880, conversational pipeline C4)

- **Barge-in = cancel-and-replan.** A `chat` message arriving while `isGenerating()` cancels the active run and replans with the new message (vs queuing). `ai-handler.ts` registers each agentic run's `AbortController` (`beginCancellableRun` / `endCancellableRun`, tagged with a monotonic id so a settling old run never clears a newer run's controller); `cancelActiveGeneration()` aborts it. The `chat` handler calls it, then `waitForGenerationIdle()` (polls `isGenerating()`) so the runs never overlap, then proceeds. The signal threads `runAgentPipeline` → `runBuildPhase`/`runMultiPageFlow`/`resumeAgentPipeline` → `runModuleDeveloper` → `createConcurrencyLimiter(n, signal)` (VIB-1880: queued tasks reject with `PipelineAbortError`; entering after abort rejects) **and** into `callAgent`'s provider request (Anthropic SDK `{ signal }` / `fetch` `signal`) so in-flight calls stop spending. Seam checks (`if (signal?.aborted) throw new PipelineAbortError()`) bail between stages. On abort the handlers return `{ canceled: true }` (mapped via `isAbortError`) instead of surfacing an error; the server emits `generation_superseded`. UI (`ui/chat.js`): the composer stays live during the build (`enableBargeIn()` on the `developing` step / `module_progress`); a barge-in send runs `supersedeCurrentRun()` (tears down the superseded run's transient UI) then sends normally. Single-call streaming and parked checkpoint gates stay locked.
- **Plan mode folded onto the gate.** Plan mode is `CheckpointKind: "plan"` — the heaviest variant. When a plan is produced, the server parks `pendingCheckpoint{kind:"plan"}` (preview carries the plan markdown; no in-memory resume store — the plan lives on `session.brandAssets.plan`). Approve / steer / cancel resolve through the same `checkpoint_resolve` WS message: the handler branches on `pending.kind` — **plan** → `runPlanApproval` (build the approved plan, one-shot) / `runPlanDeliberation` (re-stream + re-park, for steer) / `runPlanDiscard` (cancel); **design** → `handleAgenticResume`. The old `plan_approve`/`plan_discard` WS cases are thin aliases over these helpers (kept for older clients); `ui/plan.js` now sends `checkpoint_resolve {kind:"plan"}`. Build-and-commit is one shared `finalizeAgenticGeneration()` helper used by the chat, design-resume, and plan-approve paths.
